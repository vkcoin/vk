...
#if defined(NDEBUG)
# error "Vickcurrency cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Vickcurrency Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Vickcurrency: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Vickcurrency: find block wanted by given orphan block
...
// Vickcurrency: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Vickcurrency miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Vickcurrency: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Vickcurrency: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Vickcurrency: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Vickcurrency: track money supply and mint amount info
...
// Vickcurrency: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Vickcurrency: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Vickcurrency: compute stake entropy bit for stake modifier
    ...
    // Vickcurrency: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Vickcurrency: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Vickcurrency: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Vickcurrency: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Vickcurrency-loadblk");
