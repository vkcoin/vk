...
void Shutdown()
{
    ...
    RenameThread("Vickcurrency-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Vickcurrency.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Vickcurrencyd.pid)") + "\n";
...
/** Sanity checks
 *  Ensure that Vickcurrency is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Vickcurrency is shutting down."));
...
#endif
    // Make sure only a single Vickcurrency process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vickcurrency is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vickcurrency version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Vickcurrency server starting\n");
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // Vickcurrency: reserve balance amount
    {
    ...
    // requested to kill Vickcurrency during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vickcurrency") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Vickcurrency to complete") << "\n";
    ...
    else if (pwalletMain)
    {
        ...
        int count = GetArg("-genproclimit", 0);
